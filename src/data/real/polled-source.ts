...
export function createPolledSource<T>(config: PolledSourceConfig<T>): PolledSource<T> {
  ...
  return {
    ...
    note: () => {
      ...
      const ageMs = Date.now() - config.fetchedAtMs(cached);
      return ageMs > config.staleAfterMs ? config.staleNote(ageMs) : null;
    },
    poll(now, options = {}) {
      ...
      if (nowMs < blockedUntilMs) return Promise.resolve();
      if (options.force) {
        if (nowMs - lastAttemptAtMs < config.minForcedPollMs) return Promise.resolve();
      } else {
        if (nowMs < retryNotBeforeMs) return Promise.resolve();
        if (nowMs - lastAttemptAtMs < minPollMs()) return Promise.resolve();
      }
